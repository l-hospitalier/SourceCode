...
begin
  if LUDPComponent1.GetMessage(st) > 0 then
    begin
       ...
       volt := ((AD_H and $03) * $FF + AD_L) * 5/1024;     // 500/1024 = 0.48828125   0.48828125

       if boo=true then Label11.Caption:='⁂' else Label11.Caption:='';
       boo := not boo;

       volts := ' ' + FloatToStrF(volt, ffFixed, 5, 3) + 'V';
       temps := ' ' + FloatToStrF(temp, ffFixed, 5, 3) + '℃';

       if m=2 then memo1.Append(temps);

              if m = 0 then Edit1.Text := volts
         else if m = 1 then Edit2.Text := volts
         else if m = 2 then Edit3.Text := temps
         else if m = 3 then Edit4.Text := volts
         else if m = 4 then Edit13.Text := volts
         else if m = 5 then Edit14.Text := volts
         else if m = 6 then Edit15.Text := volts
         else if m = 7 then Edit16.Text := volts
    end;
    Inc(m);
  if m>7 then m:=m mod 8;
end;
...
//   C[3] := chr($02);  //Cursoe home
    for i:=2 to length(Form1.Edit9.text)+2 do
      begin
